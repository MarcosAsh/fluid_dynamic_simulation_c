...
int main(int argc, char* argv[]) {
    ...
    while ( running ) {
        ...
        if (SDL_GetTicks() - lastSprayed > 20) {
            ...
            for (int k = 0; k < 5; k++) { // Emit 10 particles per release
                angle = (angle + rand() % 360) % 360;

                int vel = rand() % 25 + 60; // Random velocity
                int density = 40;           // Density of the emitted particles
                float velX = cos(angle * PI / 180) * vel;
                float velY = sin(angle * PI / 180) * vel;

                // Emit particles from the center of the screen
                FluidSquareAddDensity(fluid, WIDTH / SCALE / 2, HEIGHT / SCALE / 2, density);
                FluidSquareAddVelocity(fluid, WIDTH / SCALE / 2, HEIGHT / SCALE / 2, velX, velY);
            }
        }
